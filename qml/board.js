...
function movePiece(piece, lastPosition, target)
{
    ...
    if (target == -1) {
        ...
        if (mode != REMOVE) {
            ...
            error = qsTrId("not_remove")
            return false
        }

        if (piece.type == player) {
            //% "That is your own piece"
            error = qsTrId("not_remove_own")
            return false
        }
        ...
        if (isPartOfMill(lastPosition)) {
            ...
            for (var i = 0; i < 3 * 8; i++) {
                if (gameBoard[i] != null && gameBoard[i].type != player && !isPartOfMill(i)) {
                    //% "This piece is part of a mill"
                    error = qsTrId("not_remove_mill")
                    return false
                }
            }
    ...
    if (mode == REMOVE && target >= 0) {
        //% "You should remove a piece"
        error = qsTrId("shall_remove")
        return false
    }
    ...
    if (piece.type != player) {
        ...
        error = qsTrId("not_own")
        return false
    }

    // No board movement during placement phase
    if (mode == PLACE && lastPosition != -1) {
        //% "You should place a piece"
        error = qsTrId("not_move_placement")
        return false
    }
...
function getPrompt() {
    if (mode == PLACE)
        //% "Place a black piece"
        return (player == BLACK) ? qsTrId("place_black")
                                 //% "Place a white piece"
                                 : qsTrId("place_white")

    if (mode == MOVE)
        //% "Move a black piece"
        return (player == BLACK) ? qsTrId("move_black")
                                 //% "Move a white piece"
                                 : qsTrId("move_white")

    if (JS.mode == REMOVE)
        //% "Take a white piece"
        return (player == BLACK) ? qsTrId("take_white")
                                 //% "Take a black piece"
                                 : qsTrId("take_black")
